...
(function($) {
    ...
    function _linkify($el, options) {
        var links = {
        		shotpro: {
                    baseUrl: URL_BASE+"/@",
                    hashtagSearchUrl: URL_BASE+"/search?q="
                },
                twitter: {
    ...
    function _linkifyUrls(matches, $el) {
        ...
        $.each(matches, function() {
            ...
            if ($el.find("a[href='" + this + "']").length === 0) {
                elContent = elContent.replace(this, "<a href='" + this + "' target='_blank' class='mentions'>" + this + "</a>");
            }
        });
    ...
    function _linkifyMentions(text, baseUrl) {
        return text.replace(/(^|\s|\(|>)@(\w+)/g, "$1<a class='mentions' href='" + baseUrl + "$2'>@$2</a>");
    }
    ...
    function _linkifyHashtags(text, links) {
        ...
        return text.replace(/(^|\s|\(|>)#((\w|[\u00A1-\uFFFF])+)/g, "$1<a class='mentions' href='" + links.hashtagSearchUrl + "$2'>#$2</a>");
    }
