<template>
    <div class="container wrap-ui content-wrapper">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <project-history v-bind:proposal="proposal"></project-history>
                <div class="panel panel-default">
                    <div class="panel-heading grid-panel-title" style="overflow: auto">
                        <router-link to="/user/messages">Messages</router-link>
                        <span v-if="! proposalLoading">> {{ proposal.project.title }}</span>

                        <span class="pull-right"
                              v-if="! proposalLoading
                                   && proposal.project.status === 'opened'
                                   && proposal.project.author_id === this.auth.id
                                   && proposal.status !== 'accepted'"
                              v-cloak
                        >
                            Offre {{ `${ proposal.offer } €` }} 
                            <!-- <a v-on:click="acceptProposal()" href="javascript:void(0)"><font color=green>Accepter</font></a> -->
                        </span>

                        <span class="pull-right" v-if="! proposalLoading && proposal.author_id === this.auth.id && proposal.status === 'opened'" v-cloak>
                            <form class="form-inline" v-on:submit.prevent="updateProposal()">
                                <fieldset v-bind:disabled="updatingProposal">
                                    <div class="input-group input-group-sm">
                                        <input v-model="proposal.offer" class="form-control input-sm" />
                                        <div class="input-group-addon">€</div>
                                    </div>
                                    <i v-show="updatingProposal" class="fa fa-spinner fa-spin fa-3x fa-fw inline"></i>
                                    <button type="submit" v-show="! updatingProposal" class="btn btn-primary btn-sm">Mettre à jour</button>
                                </fieldset>
                            </form>
                        </span>

                        <span class="pull-right text-right" v-if="! proposalLoading && proposal.status === 'accepted'" v-cloak>
                            <span>Proposition {{ proposal.status }} ({{ `${ proposal.offer } €` }})</span><br/>
                            <router-link v-bind:to="`/projects/${ proposal.project.slug }/feedback?proposal_id=${ proposal.id }`"
                            >
                                Laisser une note</router-link>
                        </span>
                    </div>
...
<script type="text/babel">
    ...
    import projectHistory from '../components/project/project-history.vue';
    ...
    export default {
        data() {
            return {
                ...
                next_page_url: `/api/messages/thread/${ this.$route.params.id }`,
                loading: false,
                errors: {},
                histories: []
            }
        ...
        created() {
            this.$events.on('acceptProposalFired', this.acceptProposal);
            this.$events.on('approveProposalFired', this.approveProposal);
            this.fetch();
            this.fetchProposal().then(() => {
        ...
        methods: {
            ...
            updateProposal() {
                this.updatingProposal = true;

                this.$http.post(`/api/proposals/${ this.proposal.id }/update`, this.proposal).then(response => {
                    this.proposal.offer = response.data.offer;
                }, response => {
                    alert(JSON.stringify(response.data));
                }).finally(() => {
                    this.updatingProposal = false;
                });
            },
            acceptProposal() {
                this.$http.post(`/api/proposals/${ this.proposal.id }/accept`).then(response => {
                    this.proposal = response.data;
                    this.$events.emit('acceptProposalCompletely');
                });
            },
            approveProposal() {
                 this.$http.post(`/api/proposals/${ this.proposal.id }/approve`).then(response => {
                    this.proposal = response.data;
                });
            },
            fetchProposal() {
                return this.$http.get(`/api/proposals/conversation/${ this.$route.params.id }`).then(response => {
                    this.proposal = response.data;
                });
            },
            onScroll(event) {
            ...
            fetch() {
                ...
                return this.$http.get(this.next_page_url).then(response => {
                    ...
                    if(this.receiver_id === null) {
                        ...
                        this.receiver_id = (this.user.id === user.sender_id) ? user.receiver_id : user.sender_id;
                    }
