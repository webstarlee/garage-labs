    ...
    <div class="form-group form-group-sm" v-bind:class="{ 'has-error': error !== undefined }">
        <fieldset v-bind:disabled="uploading">
            ...
            <div class="col-md-12 row add-btn">
                <span v-on:click="addNewDocument()" class="link">+ Ajouter un document (jpeg, png, doc(x), ppt(x), pdf, xls, csv, txt, xlsx)</span>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            upload(event, index) {
                ...
                this.$http.post('/api/startups/documents', data).then(response => {
